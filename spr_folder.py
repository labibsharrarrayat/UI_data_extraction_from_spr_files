...
import os
import sys
import spr_trend as st
import trend_graphs as tg
...
def folder_naming(f):
    return f.split('/')[-1] + '.xlsx', f.split('/')[-1] + '.png'
...
def main_process(filing):
    ...
    df = pd.DataFrame(columns=['Col1', 'Col2', 'Col3', 'Col4', 'Col5', 'Col6', 'Col7', 'Col8', 'Col9', 'Col10', 'Col11',
                               ...
                               'Col42', 'Col43', 'Col44', 'Col45', 'Col46', 'Col47', 'Col48', 'Col49', 'Col50', 'Col51',
                               'Col52'])

    df = df.append(
        {'Col1': 'Date', 'Col2': "Time", 'Col3': 'Shot Number', 'Col4': 'StartUp On', 'Col5': 'Cycl Time (s)',
         ...
         'Col31': 'Inten Tms', 'Col32': 'C F.Temp 1 Max (ßC)', 'Col33': 'C F.Temp 1 Min (ßC)',
         'Col34': 'Fill Stat (mm)',
    ...
    excel_report, graph_image = folder_naming(filing)
    print(excel_report)
    filenames = glob(filing + '/*.spr')
    print(filenames)
    ...
    for f in filenames:
        ...
        for line in lines:
            ...
            if (len(numbers_col1) >= 6) and (len(numbers_col1) < 8):
                first_columns = date_format(words)
                df = df.append({'Col1': first_columns[0], 'Col2': first_columns[1], 'Col3': words[1], 'Col4': words[2],
                                'Col5': float(str(words[3])),
                                'Col6': int(words[4]), 'Col7': float(words[5]), 'Col8': float(words[6]),
                                'Col9': int(words[7]), 'Col10': float(words[8]), 'Col11': float(words[9]),
                                'Col12': float(words[10]), 'Col13': float(words[11]), 'Col14': float(words[12]),
                                'Col15': float(words[13]), 'Col16': float(words[14]),
                                'Col17': float(words[15]), 'Col18': float(words[16]), 'Col19': float(words[17]),
                                'Col20': float(words[18]), 'Col21': float(words[19]),
                                'Col22': int(words[20]), 'Col23': float(words[21]), 'Col23': float(words[22]),
                                'Col24': float(words[23]), 'Col25': float(words[24]),
                                'Col26': float(words[25]), 'Col27': float(words[26]), 'Col28': float(words[27]),
                                'Col29': float(words[28]), 'Col30': float(words[29]),
                                'Col31': float(words[30]), 'Col32': float(words[31]), 'Col33': float(words[32]),
                                'Col34': float(words[33]), 'Col35': float(words[34]),
                                'Col36': float(words[35]), 'Col37': float(words[36]), 'Col38': float(words[37]),
                                'Col39': float(words[38]), 'Col40': float(words[39]),
                                'Col41': float(words[40]), 'Col42': float(words[41]), 'Col43': float(words[42]),
                                'Col44': float(words[43]), 'Col45': float(words[44]),
                                'Col46': float(words[45]), 'Col47': float(words[46]), 'Col48': float(words[47]),
                                'Col49': float(words[48]), 'Col50': float(words[49]),
    ...
    df.loc[
        (df['A M.Temp 1 Min (ßC)'] >= 100) & (df['A M.Temp 1 Min (ßC)'] <= 140), 'Temp_A_Min_State'] = 'Temp_A_Min_True'
    df.loc[
        (df['A M.Temp 1 Min (ßC)'] < 100) | (df['A M.Temp 1 Min (ßC)'] > 140), 'Temp_A_Min_State'] = 'Temp_A_Min_False'

    # Mold Temp, Fixed2, Min
    df.loc[
        (df['B M.Temp 2 Min (ßC)'] >= 100) & (df['B M.Temp 2 Min (ßC)'] <= 140), 'Temp_B_Min_State'] = 'Temp_B_Min_True'
    df.loc[
        (df['B M.Temp 2 Min (ßC)'] < 100) | (df['B M.Temp 2 Min (ßC)'] > 140), 'Temp_B_Min_State'] = 'Temp_B_Min_False'

    # Mold Temp, Moving1, Min
    df.loc[
        (df['C F.Temp 1 Min (ßC)'] >= 100) & (df['C F.Temp 1 Min (ßC)'] <= 140), 'Temp_C_Min_State'] = 'Temp_C_Min_True'
    df.loc[
        (df['C F.Temp 1 Min (ßC)'] < 100) | (df['C F.Temp 1 Min (ßC)'] > 140), 'Temp_C_Min_State'] = 'Temp_C_Min_False'

    # Mold Temp, Moving2, Min
    df.loc[
        (df['D F.Temp 2 Min (ßC)'] >= 100) & (df['D F.Temp 2 Min (ßC)'] <= 140), 'Temp_D_Min_State'] = 'Temp_D_Min_True'
    df.loc[
        (df['D F.Temp 2 Min (ßC)'] < 100) | (df['D F.Temp 2 Min (ßC)'] > 140), 'Temp_D_Min_State'] = 'Temp_D_Min_False'
    ...
    drop_columns = ['Cycle_Time_Value', 'L Spd Value', 'H Spd Value','H_Length_State',
                                   'Fill_Time_State','Cast_Pres_State','Biz_Size_State',
                                   'Temp_A_Min_State','Temp_B_Min_State','Temp_C_Min_State',
                                   'Temp_D_Min_State','AL_Temp_State','Vaccum_State','Pass_Status']

    excel_df = df
    excel_df = excel_df.drop(drop_columns, axis=1)

    excel_pass = pass_part
    excel_pass = excel_pass.drop(drop_columns, axis=1)


    excel_fail = fail_part
    excel_fail = excel_fail.drop(drop_columns, axis=1)

    #pass_part = pass_part.drop(df.columns[drop_columns], axis=1)
    #fail_part = fail_part.drop(df.columns[drop_columns], axis=1)
    writer = pd.ExcelWriter(filing + '/' + excel_report, engine='xlsxwriter')
    excel_df.to_excel(writer, sheet_name="All_Data", startcol=0, index=False)
    excel_pass.to_excel(writer, sheet_name="Pass_Data", startcol=0, index=False)
    excel_fail.to_excel(writer, sheet_name="Fail_Data", startcol=0, index=False)

    writer.save()

    #Plotting Graphs

    st.plot_trend1(filing + '/' + excel_report)
    #st.plot_trend2(filing + '/' + graph_image, pass_part, "pass")
    #st.plot_trend3(filing + '/' + graph_image, pass_part, "pass")
    #st.plot_trend4(filing + '/' + graph_image, pass_part, "pass")
    #st.plot_trend5(filing + '/' + graph_image, pass_part, "pass")
    #st.plot_trend6(filing + '/' + graph_image, pass_part, "pass")
    #tg.plt_trend(filing + '/' + graph_image,pass_part, "pass")


    fig = plt.figure(figsize=(40, 20))
    # axis1 = fig.add_subplot(2,3,1)

    status = ['Total Parts ' + "(" + str(df.shape[0]) + " parts" + ")",
              'Pass ' + "(" + str(pass_count) + " parts" + ")",
    ...
    if (fail_part.empty != True):
        fail_cases = ['Low Speed Fail: ' + str(fail_lspd), 'High Speed Fail: ' + str(fail_hspd),
                      'Fail Height Length: ' + str(fail_h_length),
                      ...
                      'TempC Fail: ' + str(fail_tempc), 'TempD Fail: ' + str(fail_tempd),
                      'AL Temp Fail: ' + str(fail_altemp),
        ...
        fail_labels = ['Low Speed Fail', 'High Speed Fail', 'Fail Height Length', 'Fail Fill Time',
                       'Cast Pressure Fail',
                       'Biz Size Fail', 'TempA Fail', 'TempB Fail', 'TempC Fail', 'TempD Fail', 'AL Temp Fail',
                       'Vaccum Fail']

        colors = (
        'tab:olive', 'tab:red', 'tab:cyan', 'forestgreen', 'tab:pink', 'tab:gray', 'tab:purple', 'tab:orange', 'navy',
        'tab:brown', 'indigo', 'gold')

        # fig = plt.figure(figsize=(60,20))
        axis2 = fig.add_subplot(2, 2, 2)

        for axis in [axis2.yaxis]:
            axis.set_major_locator(ticker.MaxNLocator(integer=True))

        axis2.bar(fail_cases, fail_status, color=colors)
        plt.title('Fail Parts on ' + date_file[1] + " at " + time_file[1] +
                  " to " + date_file[df.shape[0]-1] + " at " + time_file[df.shape[0]-1])
        plt.xticks(rotation=90)
        plt.xlabel("Case of Failure")
        plt.ylabel('Number of Parts')

        axis3 = fig.add_subplot(2, 2, 3)
        axis3.pie(fail_status, autopct='%1.2f%%',colors=colors)
        axis3.legend(fail_labels, loc="upper right")
        plt.title('Percentage of Fail Cases ' + date_file[1] + " at " + time_file[1] +
                  " to " + date_file[df.shape[0]-1] + " at " + time_file[df.shape[0]-1])

    fig.savefig(filing + '/' + graph_image)

    os.startfile(filing + '/' + excel_report)
    os.startfile(filing + '/' + graph_image)
    st.open_pdf()
